...
\begin{equation}
 	dL(x,\omega_{0})=-\kappa_a(x)*L(x,\omega_{0})dx
 \end{equation}
...
\begin{equation}
\label{eq:INSCATT}
 	dL(x,\omega_{0})=\frac{\kappa_t(x)}{4\pi}*\int_{\Omega}L(x,\omega_{i})p(x,\omega_{0},\omega_{i})d\omega_{i}
 \end{equation}
...
\label{sec:EXTREND}
This equation describes the energy loss caused by a differential volume, to integrate overall loss over a longer segments of the media we have to compute its \bd{optical thickness} as fallows:
\begin{equation}
